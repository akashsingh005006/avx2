...
import ticketSalesAbi from "../artifacts/contracts/TicketSales.sol/TicketSales.json";

export default function App() {
  const [ethWallet, setEthWallet] = useState(undefined);
  const [account, setAccount] = useState(undefined);
  const [ticketSalesSigner, setTicketSalesSigner] = useState(undefined);
  const [ticketsEntered, setTicketsEntered] = useState(0);
  ...
  const [ticketsSold, setTicketsSold] = useState(undefined);

  const contractAddress = "0x5FbDB2315678afecb367f032d93F642f64180aa3";
  const ticketSalesABI = ticketSalesAbi.abi;
  ...
  const handleAccount = (accounts) => {
    if (accounts.length > 0) {
      ...
      getTicketSalesContract(accounts[0]);
    } else {
  ...
  const getTicketSalesContract = async (account) => {
    console.log("before signer");
    const provider = new ethers.BrowserProvider(window.ethereum);
    const signer = await provider.getSigner(account);
    console.log("after signer");

    const ticketSalesContractSigner = new ethers.Contract(
      contractAddress,
      ticketSalesABI,
      signer
    );
    console.log("after contract");
    setTicketSalesSigner(ticketSalesContractSigner);
    console.log(ticketSalesContractSigner);
  };
  ...
  const buyTicket = async () => {
    if (
      ...
      amountEntered > 0
      &&
      ethers.toBigInt(amountEntered) >= ethers.toBigInt(ticketPrice) * ethers.toBigInt(ticketsEntered)
    ) {
      try {
        // const valueInWei = ethers.parseUnits(amountEntered.toString(), "ether");
        // console.log(valueInWei);
        // console.log(amountEntered);
        // console.log(ethers.parseEther("1.0"));

        let tx = await ticketSalesSigner.buyTicket(ticketsEntered, {
          value: amountEntered,
      ...
      console.error(
        "ticketSalesSigner is not set or amountEntered is not Insufficient."
      );
    }
  };

  const getEventName = async () => {
    if (ticketSalesSigner) {
      const NAME = await ticketSalesSigner.getEventTitle();
      setEventName(NAME);
    }
  };
  ...
  const getOrganizer = async () => {
    if (ticketSalesSigner) {
      const ORGANIZER = await ticketSalesSigner.getOrganizer();
  ...
  const getTicketPrice = async () => {
    if (ticketSalesSigner) {
      const PRICE = await ticketSalesSigner.getTicketPrice();
  ...
  const getTotalTickets = async () => {
    if (ticketSalesSigner) {
      const TOTAL_TICKETS = await ticketSalesSigner.getTotalTickets();
  ...
  const getTicketsSold = async () => {
    if (ticketSalesSigner) {
      const TICKETS_SOLD = await ticketSalesSigner.getTicketsSold();
      setTicketsSold(TICKETS_SOLD);
    }
  };
    ...
    return (
      <div className="flex flex-col justify-center items-center w-full h-full gap-3">
        <p className="text-white font-bold text-5xl">
          Event Name: {eventName || "Loading..."}
        </p>
        <p className="text-gray-300  text-2xl">
          Organizer address:{" "}
          {organizer !== undefined ? `${organizer}` : "Loading..."}
        </p>
        <p className="text-gray-300  text-2xl">
          Ticket Price:{" "}
          {ticketPrice !== undefined ? `${ticketPrice} WEI` : "Loading..."}
        </p>
        <p className="text-gray-300  text-2xl">
          Total Tickets:{" "}
          {totalTickets !== undefined ? `${totalTickets} WEI` : "Loading..."}
        </p>
        <p className="text-gray-300  text-2xl">
          Tickets Sold Out:{" "}
          {ticketsSold !== undefined ? `${ticketsSold}` : "Loading..."}
        </p>
        <input
        ...
        />
        <input
          placeholder="Enter the number of tickets to buy"
          className="border-2 border-gray-400 my-5 text-xl bg-[#08083b] rounded-full px-4 py-3 placeholder:text-gray-400 text-gray-200 focus:outline-none focus:ring-2 focus:ring-[#0a0a89] focus:border-white focus:border-[3px]"
          onChange={(e) => setTicketsEntered(e.target.value)}
        />
        <button
          onClick={buyTicket}
